...
import os
# from models.proposed_model import CustomModel #v1
from models.branch_model import CustomModel #v2
from models.heads import *
# from models import CustomModel
from aux import optimizers as optims
from aux import schedulers as schedulers
from aux import losses as losses ##v1
# from aux import smp_losses as losses
...
def main():
    ...
    if type(train_data_path) != list:
        ...
        for i in train_data_path:
            ...
            train_label_paths.extend(glob(os.path.join(i, "label_images", "*")))
    ...
    if type(val_data_path) != list:
        ...
        for i in val_data_path:
            ...
            val_label_paths.extend(glob(os.path.join(i, "label_images", "*")))
        val_img_paths.sort()
    ...
    train_transform_weak = Augmenter(**train_augprams_weak, img_size=config['train']['dataloader']['img_size'])
    train_l_loader, train_u_loader = get_loader(
        train_img_paths,
        train_label_paths,
        transforms=train_transform,
        transforms_weak = train_transform_weak,
    ...
    if pretrained == '' or pretrained is None:
        ...
        model = CustomModel(backbone=str(backbone), decode=str(head), num_classes=num_classes, pretrained=pretrained, memory_size=memory_size)
    else:
        try:
            ...
            model = CustomModel(backbone=str(backbone), decode=str(head), num_classes=num_classes, pretrained=None, memory_size=memory_size)
    ...
    trainer = Trainer(
        model, model_name, optimizer, loss, scheduler, save_dir, save_from, logger, device, config['model']['strong_threshold'], config['model']['weak_threshold'], config['model']['num_queries'], config['model']['num_negatives'], use_amp=use_amp, use_ddp=use_ddp, multi_loss=use_multi_loss, name_writer=name_writer
    )
