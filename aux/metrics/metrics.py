...
class MetricForNeoplasm():
    ...
    def cal(self, predict, mask, ignore=None, smooth=1):
        ...
        return np.array(self.list_dice_score).mean(), np.array(self.list_iou_score).mean(), dice_score, iou_score
    ...
    def show(self):
        ...
        logger.info("Evaluate {}".format(self.name))
        logger.info('Macro scores: Dice: {:.3f} | IOU: {:.3f}'.format(
            np.array(self.list_dice_score).mean(),
            np.array(self.list_iou_score).mean()
        ))
        logger.info('Micro scores: Dice: {:.3f} | IOU: {:.3f}'.format(
            dice_score,
            iou_score
        ))
