...
class Augmenter(nn.Module):
    def __init__(
        ...
        #     A.RandomResizedCrop(220, 220, p=0.3)
        # ])
        ...
        self.transforms = A.Compose(
            [
                ...
                #     p=crop_prob,
                # ),
                A.RandomResizedCrop(self.img_size, self.img_size, p=crop_prob),
                ElasticTransform(
            ...
            p=self.prob,
            additional_targets={'neo': 'mask', 'non': 'mask'}
            
        )

    def forward(self, image, neo, non, softlabel=None):
        if softlabel is None:
            result = self.transforms(image=image, neo=neo, non=non)
        else:
            result = self.transforms(image=image, neo=neo, non=non, softlabel=softlabel)
