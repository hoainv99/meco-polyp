...
import os
# from models.custom_model import CustomModel #v1
...
def main():
    ...
    if type(train_data_path) != list:
        ...
        for i in train_data_path:
            train_img_paths.extend(glob(os.path.join(i, "train/train", "*")))
            train_label_paths.extend(glob(os.path.join(i, "train_gt/train_gt", "*")))
    ...
    if type(val_data_path) != list:
        ...
        for i in val_data_path:
            ...
            val_label_paths.extend(glob(os.path.join(i, "labels", "*")))
        val_img_paths.sort()
    ...
    train_augprams_weak = config["train"]["augment_weak"]
    ...
    trainer = Trainer(
        model, model_name, optimizer, loss, scheduler, save_dir, save_from, logger, device, use_amp=use_amp, use_ddp=use_ddp, multi_loss=use_multi_loss, name_writer=name_writer
    )
    val_loader = None
    trainer.train_loop(
        train_l_loader,\
