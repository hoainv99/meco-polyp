...
from utilizes.dataloader import get_loader, get_val_loader
from utilizes.augment import Augmenter
import tqdm   
import torch
from loguru import logger
# from models.proposed_model import CustomModel
...
def main():
    ...
    gts = []
    prs = []
    df = pd.DataFrame(columns=['dataset', 'mDice', 'mIOU'])
    datasets = config["dataset"]["test_data_path"]
    for data_path in datasets:
        ...
        test_img_paths.extend(glob(os.path.join(test_data_path, "images", "*")))
        test_mask_paths.extend(glob(os.path.join(test_data_path, "masks", "*")))
    ...
    arch = config['model']['save_dir'].split('/')[-1]

    df['dataset'] = dts
    df['mDice'] = mDice
    df['mIOU'] = mIOU
    csv_save_dir = os.path.join(config['test']['csv'], arch)
    if not os.path.exists(csv_save_dir):
        os.makedirs(csv_save_dir)
    df.to_csv(f'{csv_save_dir}/result.csv', index=False)
    return gts, prs
